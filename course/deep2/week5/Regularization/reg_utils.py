...
# These appear to be unused, now passes Flake8
# import sklearn
...
def load_planar_dataset(randomness, seed):
    ...
    # Note this is never used and repeated
...
def plot_decision_boundary(model, X, y):
    ...
    plt.ylabel("x2")
    plt.xlabel("x1")
    # Need to squeeze when using y as the colors
...
def load_2D_dataset():
    ...
    test_X = data["Xval"].T
    test_Y = data["yval"].T
    # need to squeeze for compatiblity with matplotlib versions greater than 2
    plt.scatter(
        train_X[0, :],
        train_X[1, :],
        c=np.squeeze(train_Y),
        s=40,
